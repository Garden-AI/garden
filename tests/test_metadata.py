...
pip_requirements = {"format": "pip", "contents": ["scikit-learn==1.2.2", "pandas"]}
...
def test_add_metadata_cell(mocker):
    ...
    mocker.patch("garden_ai.metadata._read_notebook", return_value=ntbk)

    nbformat_write_mock = mocker.patch("garden_ai.metadata.nbformat.write")
...
def test_get_metadata(mocker):
    ...
    notebook_metadata = get_notebook_metadata(None)
...
def test_set_metadata(mocker):
    ...
    set_notebook_metadata(
        ...
        "3.9-base",
        "A_BASE_IMAGE_URI",
        {"format": "pip", "contents": ["scikit-learn==1.2.2", "pandas"]},
    )
...
def test_read_requirements(mocker):
    ...
    mocker.patch(
        "garden_ai.metadata.get_notebook_metadata", return_value=notebook_metadata_pip
    )

    mocker.patch("pathlib.Path.is_file", return_value=True)

    # pip file
    with patch("builtins.open", mock_open(read_data=pip_requirements_raw)) as mock_file:
        requirements_data = read_requirements_data(Path("file.txt"), Path("notebook"))
        assert requirements_data == pip_requirements

    # conda file
    with patch(
        "builtins.open", mock_open(read_data=conda_requirements_raw)
    ) as mock_file:
        requirements_data = read_requirements_data(Path("file.yaml"), Path("notebook"))
        assert requirements_data == conda_requirements

    # no file, from notebook
    requirements_data = read_requirements_data(
        Path("file.badextension"), Path("notebook")
    )
    assert requirements_data == pip_requirements
...
def test_write_requirements(mocker):
    ...
    with patch("builtins.open", mock_open()) as mock_file:
        save_requirements_data(Path("file.txt"), pip_requirements)
        mock_file().write.assert_called_with(pip_requirements_raw)

    with patch("builtins.open", mock_open()) as mock_file:
        yaml_dump_mock = mocker.patch("garden_ai.metadata.yaml.dump", return_value=True)
        save_requirements_data(Path("file.yaml"), conda_requirements)
