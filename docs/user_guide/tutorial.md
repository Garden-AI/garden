...
This command will register your model with the Garden service and return a "full name" URI, consisting of your email, your model's name, and a version, e.g. `"me@institution.edu-my-model-name/1"`. This full model name is how you can reference the registered model in your pipeline code:
...
Once we're satisfied with our pipeline's definition in `pipeline.py`, we can register the pipeline, allowing us to:
 ...
 - Enable the pipeline to execute on a remote Globus Compute endpoint.
...
You can add your registered pipeline to your newly created garden like so:
...
You can find a published Garden by searching for it using the CLI. This would list all published Gardens with `"Dee, Tweedle"` listed as an author:
```bash
garden-ai garden search --author "Dee, Tweedle"
```

Grabbing just the DOI from the output of that command, we have everything we need to execute the pipeline on a choice Globus Compute endpoint:
...
That's all there is to it! You've just developed and published your own Garden, and learned how to use published Gardens to remotely execute and reproduce your work. Enjoy working with the Garden framework!
